import { TableBody, TableRow, TableCell, Button, Table, TableHeader } from "@roketid/windmill-react-ui";
...
const SertifikatDetail: React.FC<any> = ({ role, sertifikat }: { role: string, sertifikat: Sertifikat }) => {
        ...
        <Table>
          <TableBody>
            {Object.keys(sertifikat).map((key) => {
              ...
              if (key === 'created_at' || key === 'diambil' || key === 'id') return ""
              else if (!sertifikat.diambil && key === 'tanggal_pengambilan' || key === 'nama_penerima' || key === 'nik_penerima') return ""
              else if (key === 'daerah') return (
                <TableRow
                  key={key}
                >
                  <TableCell>
                  ...
                  <TableCell>
                    <span className="text-sm">{sertifikat['daerah']['kecamatan'] + "/" + sertifikat['daerah']['desa']}</span>
                  </TableCell>
                </TableRow>
              )
              else
                return (
                  ...
                  >
                    ...
                    <TableCell>
                      <span className="text-sm">{sertifikat[key as keyof Sertifikat]}</span>
                    </TableCell>
                  </TableRow>
                )
            })}
